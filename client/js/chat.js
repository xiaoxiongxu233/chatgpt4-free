...
const ask_gpt = async (message) => {
    try {
            ...
            body: JSON.stringify({
                ...
                meta        : {
                    ...
                    content : {
                        ...
                        internet_access :  document.getElementById("internet-switch").checked,
                        is_creative_bot :  document.getElementById("creative-switch").checked,
                        is_detail_bot   :  document.getElementById("detail-switch").checked,
                        content_type    : "text",
                        parts           : [{
                            content: message,
                            role   : "user"
        ...
        const elementGPT    = document.getElementById(`gpt_${window.token}`);
        var currentPhase    = 0;
        var text            = '';
        ...
        while (true) {
            ...
            elementGPT.innerHTML = markdown.render(text)

            /* Animation
            switch (currentPhase) {
                case 1:
                    elementGPT.innerHTML = markdown.render("Печатает.")
                    currentPhase++;
                    break;
                case 2:
                    elementGPT.innerHTML = markdown.render("Печатает..")
        ...
        if (text.includes(`instead. Maintaining this website and API costs a lot of money`)) {
            elementGPT.innerHTML = 'An error occured, please reload / refresh cache and try again.'
        }
        ...
        if (e.name != `AbortError`) {
            ...
            elementGPT.innerHTML = error_message
            add_message(window.conversation_id, 'assistant', error_message)
        } else {
            elementGPT.innerHTML += ` [Остановлено]`
            add_message(window.conversation_id, 'assistant', text + ` [Остановлено]`)
        }
...
window.onload = async () => {
    document.getElementById("internet-switch").checked  = true
